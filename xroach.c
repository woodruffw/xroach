/*
    ...
    Copyright 2017 by William Woodruff
...
char Copyright[] = "Xroach\nCopyright 1991 J.T. Anderson, 2018 William Woodruff";
...
void main(int ac, char *av[])
{
    ...
    */
    for (ax = 0; ax < 360; ax += ROACH_ANGLE) {
        ...
        rp = &roachPix[rx];
        rp->pixmap = XCreateBitmapFromData(display, rootWin, rp->roachBits,
                                           rp->width, rp->height);
        rp->sine = sin(angle);
        rp->cosine = cos(angle);
    }
...
*/
int RoachInRect(Roach *roach, int rx, int ry, int x, int y,
...
*/
int RoachOverRect(Roach *roach, int rx, int ry, int x, int y,
...
void MoveRoach(int rx)
{
    ...
    if (RoachInRect(roach, (int)newX, (int)newY, 0, 0, display_width,
                    display_height)) {
        roach->x = newX;
        roach->y = newY;
        ...
        for (ii = rx + 1; ii < curRoaches; ii++) {
            r2 = &roaches[ii];
            if (RoachOverRect(roach, (int)newX, (int)newY, r2->intX, r2->intY,
                              r2->rp->width, r2->rp->height)) {
                TurnRoach(roach);
            }
        }
    } else {
...
void DrawRoaches()
{
    ...
    for (rx = 0; rx < curRoaches; rx++) {
        ...
        if (!roach->hidden) {
            ...
            XFillRectangle(display, rootWin, gc, roach->intX, roach->intY,
                           roach->rp->width, roach->rp->height);
        } else {
            roach->intX = -1;
        }
    }
...
void CoverRoot()
{
    ...
    roachWin = XCreateWindow(display, rootWin, 0, 0, display_width,
                             display_height, 0, CopyFromParent, InputOutput,
                             CopyFromParent, wamask, &xswa);
    ...
    */
    ...
    for (wx = 0; wx < nChildren; wx++) {
        ...
        if (wa.map_state == IsViewable) {
            XGetGeometry(display, children[wx], &dummy, &winX, &winY, &winWidth,
...
int MarkHiddenRoaches()
{
    ...
    for (rx = 0; rx < curRoaches; rx++) {
        ...
        if (!r->hidden) {
            if (r->intX > 0 &&
                XRectInRegion(rootVisible, r->intX, r->intY, r->rp->width,
                              r->rp->height) == RectangleOut) {
                r->hidden = 1;
            } else {
                nVisible++;
            }
...
Pixel AllocNamedColor(char *colorName, Pixel dfltPix)
{
    ...
    if (XAllocNamedColor(display, DefaultColormap(display, screen), colorName,
                         &scrncolor, &exactcolor)) {
        pix = scrncolor.pixel;
    } else {
        pix = dfltPix;
    }
